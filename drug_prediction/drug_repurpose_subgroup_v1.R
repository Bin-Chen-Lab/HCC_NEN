subgroup = "alcohol"
subgroup_full = "Alcohol_consumption"
subset_comparison_id = "lihc_alcohol_all_non_tumors"
setwd(paste("/Users/binchen1/Documents/stanford/hcc/data/LIHC/drug_subgroup_v1/", subgroup, sep=""))
...
signatures = read.table(paste("LIHC", subgroup_full, "_dz_signature_cmap_final_all_tumor_15_1E-2.txt", sep=""), header=T,sep="\t") #at least cover gene id and value that can be either fold change or p value
...
query <- paste("select experiment_id, name from proj_repositioning.v_cmap_hits_valid  where valid=1 and subset_comparison_id = '", subset_comparison_id, "' order by cmap_score  "  , sep=""  )         
rs <- dbSendQuery(con, query)    
...
##################
#violin plot 
drugs = unique(drug_instances_all$name[drug_instances_all$q_value < 0.01 & drug_instances_all$cmap_score < 0 & tolower(drug_instances_all$name) %in% c(tolower(drugbank_drugs), "pyrvinium")])

#signicant cutoff
sig_cutoff = max(drug_instances_all$cmap_score[drug_instances_all$q_value < 0.01 & drug_instances_all$cmap_score < 0])
...
for (i in 1:length(drugs)){
  ...
  ratios = c(ratios, nrow(subset(cmap_predictions_drug, cmap_score < sig_cutoff & p_value < 0.05)) / nrow(cmap_predictions_drug))
...
candidates = drug_names #head(drugs_rank, 20)$drugs
hits_scores = subset(drug_instances_all, name %in% candidates, select = c("cmap_score", "name"))
...
createGGPlot(scores, measureVar="score", groupVars = "group", main = "", ylab = "Score", xlab = "")
dev.off()
...
signatures = read.table(paste("LIHC", subgroup_full, "_dz_signature_lincs_final_all_tumor_15_1E-2.txt", sep=""), header=T,sep="\t") #at least cover gene id and value that can be either fold change or p value
signatures$value =  signatures$value #norm vs control
...
rs <- dbSendQuery(con, paste("select * from proj_lincs.v_lincs_predictions_drugs where cell_id in ('HEPG2', 'HUH7') and subset_comparison_id ='", subset_comparison_id, "'", sep=""))    
lincs_predictions <- fetch(rs, n = -1)
lincs_predictions$cmap_score = - lincs_predictions$cmap_score
lincs_predictions = lincs_predictions[order(lincs_predictions$cmap_score),] #there was one error in building dz signatures, the label "up" and "down" were reversed. so here, only look the high scored drugs
...
plot(density(lincs_predictions$cmap_score), col = 1, xlab="LINCS score", main="")
candidates = drug_names #as.character(drugs_rank$drugs[1:10]) #c("sotalol", "mebendazole", "brimonidine", "epibatidine")
...
#find common drugs 
query <- paste("select * from proj_repositioning.v_cmap_hits_valid  where cmap_score < 0 and valid=1 and subset_comparison_id = '", subset_comparison_id, "' and q_value < 0.05 order by cmap_score  " , sep=""   )         
rs <- dbSendQuery(con, query)    
...
rs <- dbSendQuery(con, paste("select experiment_id, cmap_score, q_value,q_value, sig_id,pert_desc, pert_time,pert_dose, pert_iname, cell_id from proj_lincs.v_lincs_predictions_drugs where cmap_score>0 and q_value < 0.05 and subset_comparison_id ='", subset_comparison_id, "' order by cmap_score", sep=""))    
lincs_results <- fetch(rs, n = -1)
write.csv(lincs_results, "lincs_results.csv")

rs <- dbSendQuery(con, paste("select experiment_id, cmap_score, q_value,q_value, sig_id,pert_desc, pert_time,pert_dose, pert_iname, cell_id from proj_lincs.v_lincs_predictions_all where cell_id in ('HEPG2', 'HUH7') and cmap_score>0 and q_value < 0.05 and subset_comparison_id ='", subset_comparison_id, "' order by cmap_score", sep=""))    
