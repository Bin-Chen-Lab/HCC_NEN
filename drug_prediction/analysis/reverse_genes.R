...
pdf("hcc_nen_niclosamide_whole.pdf")
  colPal <- redgreen(100)
...
drug_dz_signature_rank = drug_dz_signature_rank[order(drug_dz_signature_rank$value), ]
pdf("hcc_nen_niclosamide_pdx.pdf")
  colPal <- redgreen(100)
  par(mar=c(12, 4, 2, 0.5))
  #image(t(druggable_targets_pos), col=redblue(2))
  image(t(drug_dz_signature_rank), col= colPal,   axes=F, srt=45)
...
write.csv(gene_reversed, "../../microarray/reversed_genes.csv")
...
pdf("dz_induced_by_NEN_niclo.pdf")
...
pdf("dz_suppressed_by_NEN_niclo.pdf")
...
#cluster drug expression usign reversed genes signatures
data2 = read.csv("../../microarray//Group3/data2.csv")
data2_expr = subset(data2, GENE_ID %in% gene_reversed$GeneID[p_up_adj < 0.1 | p_down_adj < 0.1] & fail.count>=0, select=c("GeneSymbol", "N_576", "N_577", "N_578", "N_596", "N_597", "N_598"))
data2_expr = aggregate(. ~ GeneSymbol, data2_expr, mean)
rownames(data2_expr) = data2_expr$GeneSymbol
data2_expr = data2_expr[,-1]
annotation = data.frame(type=c("control", "control", "control", "NEN", "NEN", "NEN"))
...
my.cols <- greenred(100) #brewer.pal(9, "Blues")
pheatmap((data2_expr), col = my.cols, annotation = annotation,   
         show_colnames=T, legend=T, show_rownames=T
)
...
#################################################
#overlap genes between in vitro and in vivo
g2_NEN_DEG_file = loadWorkbook("../../microarray/Group1/data3-sig_G2-NEN_vs_G2.xlsx")
g2_NEN_DEG = readWorksheet(g2_NEN_DEG_file, sheet = 1, header = TRUE)

g2_Niclo_DEG_file = loadWorkbook("../../microarray/Group1/data3-sig_G2-Niclo_vs_G2.xlsx")
g2_Niclo_DEG = readWorksheet(g2_Niclo_DEG_file, sheet = 1, header = TRUE)

h7_NEN_DEG_file = loadWorkbook("../../microarray/Group2/data3-sig_H7-NEN_vs_H7.xlsx")
h7_NEN_DEG = readWorksheet(h7_NEN_DEG_file, sheet = 1, header = TRUE)

h7_Niclo_DEG_file = loadWorkbook("../../microarray/Group2/data3-sig_H7-Niclo_vs_H7.xlsx")
h7_Niclo_DEG = readWorksheet(h7_Niclo_DEG_file, sheet = 1, header = TRUE)

pdx_DEG_file = loadWorkbook("../../microarray/Group3/data3.xlsx")
pdx_DEG = readWorksheet(pdx_DEG_file, sheet = 1, header = TRUE)

intersect(intersect(g2_NEN_DEG$GeneSymbol, g2_Niclo_DEG$GeneSymbol), pdx_DEG$GeneSymbol)
intersect(intersect(h7_NEN_DEG$GeneSymbol, h7_Niclo_DEG$GeneSymbol), pdx_DEG$GeneSymbol)

Reduce(intersect, list(g2_NEN_DEG$GeneSymbol, g2_Niclo_DEG$GeneSymbol, pdx_DEG$GeneSymbol,h7_NEN_DEG$GeneSymbol, h7_Niclo_DEG$GeneSymbol))

setlist <- list(NEN_HepG2 =as.character(unique(g2_NEN_DEG$GeneSymbol)),                 
                NEN_PDX =as.character(unique(pdx_DEG$GeneSymbol)),
                Niclo_HepG2 =as.character(unique(g2_Niclo_DEG$GeneSymbol))) # NEN_Huh7 =as.character(unique(h7_NEN_DEG$GeneSymbol)), ,Niclo_Huh7 =as.character(unique(h7_Niclo_DEG$GeneSymbol)) ,C=unique(set1$GeneID[set1$up_down=="down"]), D=unique(set2$GeneID[set2$up_down=="down"]))
setlist2 <- setlist[1:3]
OLlist2 <- overLapper(setlist=setlist2, sep="_", type="vennsets")
OLlist2$Venn_List
counts <- sapply(OLlist2$Venn_List, length); 
pdf("../../microarray/gsea/common_genes.pdf")
  vennPlot(counts=counts) 
dev.off()
#"ASNS"  "CEBPB" "EGR1"  "HAMP" "SLC6A9"
subset(pdx_DEG, GeneSymbol %in% c("ASNS",  "CEBPB", "EGR1" , "HAMP", "SLC6A9"))

#GSEA enrichment
#canonical pathway down
NEN_G2_CP_V5 = read.csv("../../microarray/gsea/NEN_G2_CP_V5.GseaPreranked.1441685822722/gsea_report_for_na_neg_1441685822722.csv")
NEN_H7_CP_V5 = read.csv("../../microarray/gsea/NEN_H7_CP_V5.GseaPreranked.1441686429063/gsea_report_for_na_neg_1441686429063.csv")
NEN_PDX_CP_V5 = read.csv("../../microarray/gsea/NEN_PDX_CP_V5.GseaPreranked.1441685482288/gsea_report_for_na_neg_1441685482288.csv")
Niclo_G2_CP_V5 = read.csv("../../microarray/gsea/Niclo_G2_CP_V5.GseaPreranked.1441686170541/gsea_report_for_na_neg_1441686170541.csv")
Niclo_H7_CP_V5 = read.csv("../../microarray/gsea/Niclo_H7_CP_V5.GseaPreranked.1441686662034/gsea_report_for_na_neg_1441686662034.csv")

cutoff = 0.05
NEN_G2_CP_V5 = subset(NEN_G2_CP_V5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
NEN_H7_CP_V5 = subset(NEN_H7_CP_V5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
NEN_PDX_CP_V5 = subset(NEN_PDX_CP_V5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
Niclo_G2_CP_V5 = subset(Niclo_G2_CP_V5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
Niclo_H7_CP_V5 = subset(Niclo_H7_CP_V5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))

common_pathways = Reduce(intersect, list(NEN_G2_CP_V5$NAME, NEN_H7_CP_V5$NAME, NEN_PDX_CP_V5$NAME, Niclo_G2_CP_V5$NAME, Niclo_H7_CP_V5$NAME ))

setlist <- list(NEN_HepG2 =as.character(unique(NEN_G2_CP_V5$NAME)),                 
                NEN_PDX =as.character(unique(NEN_PDX_CP_V5$NAME)),
                Niclo_HepG2 =as.character(unique(Niclo_G2_CP_V5$NAME))
                ) # Niclo_Huh7 =as.character(unique(Niclo_H7_CP_V5$NAME)) NEN_Huh7 =as.character(unique(NEN_H7_CP_V5$NAME)),,C=unique(set1$GeneID[set1$up_down=="down"]), D=unique(set2$GeneID[set2$up_down=="down"]))
setlist2 <- setlist[1:3]
OLlist2 <- overLapper(setlist=setlist2, sep="_", type="vennsets")
OLlist2$Venn_List
counts <- sapply(OLlist2$Venn_List, length); 
pdf("../../microarray/gsea/common_canonical_pathway_neg.pdf")
  vennPlot(counts=counts) 
dev.off()

NEN_PDX_CP_V5 = NEN_PDX_CP_V5[order(NEN_PDX_CP_V5$FDR.q.val),]
subset(NEN_PDX_CP_V5, NAME %in% common_pathways)$NAME

names(NEN_G2_CP_V5) = c("NAME", "ES_NEN_HepG2", "NES_NEN_HepG2", "NOM.p.val_NEN_HepG2", "FDR.q.val_NEN_HepG2")
names(Niclo_G2_CP_V5) = c("NAME", "ES_Niclo_HepG2", "NES_Niclo_HepG2", "NOM.p.val_Niclo_HepG2", "FDR.q.val_Niclo_HepG2")
names(NEN_PDX_CP_V5) = c("NAME", "ES_NEN_PDHX", "NES_NEN_PDHX", "NOM.p.val_NEN_PDHX", "FDR.q.val_NEN_PDHX")

merged_common_canonical_pathway_neg = merge(merge(NEN_PDX_CP_V5,NEN_G2_CP_V5, by="NAME" ), Niclo_G2_CP_V5, by="NAME")
merged_common_canonical_pathway_neg = merged_common_canonical_pathway_neg[order(merged_common_canonical_pathway_neg$FDR.q.val_NEN_PDHX),]
write.csv(merged_common_canonical_pathway_neg, "../../microarray/gsea/common_canonical_pathway_neg.csv")

#cannonical up
NEN_G2_CP_V5 = read.csv("../../microarray/gsea/NEN_G2_CP_V5.GseaPreranked.1441685822722/gsea_report_for_na_pos_1441685822722.csv")
NEN_H7_CP_V5 = read.csv("../../microarray/gsea/NEN_H7_CP_V5.GseaPreranked.1441686429063/gsea_report_for_na_pos_1441686429063.csv")
NEN_PDX_CP_V5 = read.csv("../../microarray/gsea/NEN_PDX_CP_V5.GseaPreranked.1441685482288/gsea_report_for_na_pos_1441685482288.csv")
Niclo_G2_CP_V5 = read.csv("../../microarray/gsea/Niclo_G2_CP_V5.GseaPreranked.1441686170541/gsea_report_for_na_pos_1441686170541.csv")
Niclo_H7_CP_V5 = read.csv("../../microarray/gsea/Niclo_H7_CP_V5.GseaPreranked.1441686662034/gsea_report_for_na_pos_1441686662034.csv")
...
setlist <- list(NEN_HepG2 =as.character(unique(NEN_G2_CP_V5$NAME)), 
                NEN_Huh7 =as.character(unique(NEN_H7_CP_V5$NAME)),
                NEN_PDX =as.character(unique(NEN_PDX_CP_V5$NAME)),
                Niclo_HepG2 =as.character(unique(Niclo_G2_CP_V5$NAME)),
                Niclo_Huh7 =as.character(unique(Niclo_H7_CP_V5$NAME))) # ,C=unique(set1$GeneID[set1$up_down=="down"]), D=unique(set2$GeneID[set2$up_down=="down"]))
setlist2 <- setlist[1:5]
OLlist2 <- overLapper(setlist=setlist2, sep="_", type="vennsets")
OLlist2$Venn_List
counts <- sapply(OLlist2$Venn_List, length); 
pdf("../../microarray/gsea/common_canonical_pathway_pos.pdf")
vennPlot(counts=counts) 
dev.off()
...
merged_common_canonical_pathway_neg = merge(merge(NEN_PDX_CP_V5,NEN_G2_CP_V5, by="NAME" ), Niclo_G2_CP_V5, by="NAME")
merged_common_canonical_pathway_neg = merged_common_canonical_pathway_neg[order(merged_common_canonical_pathway_neg$FDR.q.val_NEN_PDHX),]
write.csv(merged_common_canonical_pathway_neg, "../../microarray/gsea/common_canonical_pathway_pos.csv")



#GO
NEN_G2_C5= read.csv("../../microarray/gsea/NEN_G2_c5.GseaPreranked.1441689122447/gsea_report_for_na_neg_1441689122447.csv")
NEN_H7_C5  = read.csv("../../microarray/gsea/NEN_G2_c5.GseaPreranked.1441689122447/gsea_report_for_na_neg_1441689122447.csv")
NEN_PDX_C5 = read.csv("../../microarray/gsea/NEN_PDX_c5.GseaPreranked.1441688600455/gsea_report_for_na_neg_1441688600455.csv")
Niclo_G2_C5 = read.csv("../../microarray/gsea/Niclo_G2_c5.GseaPreranked.1441689438618/gsea_report_for_na_neg_1441689438618.csv")
Niclo_H7_C5 = read.csv("../../microarray/gsea/Niclo_h7_c5.GseaPreranked.1441690000502/gsea_report_for_na_neg_1441690000502.csv")

cutoff = 0.05
NEN_G2_C5 = subset(NEN_G2_C5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
NEN_H7_C5 = subset(NEN_H7_C5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
NEN_PDX_C5 = subset(NEN_PDX_C5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
Niclo_G2_C5 = subset(Niclo_G2_C5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))
Niclo_H7_C5 = subset(Niclo_H7_C5, FDR.q.val < cutoff, select=c("NAME", "ES", "NES", "NOM.p.val", "FDR.q.val"))

common_pathways=Reduce(intersect, list(NEN_G2_C5$NAME, NEN_H7_C5$NAME, NEN_PDX_C5$NAME, Niclo_G2_C5$NAME, Niclo_H7_C5$NAME ))

setlist <- list(NEN_HepG2 =as.character(unique(NEN_G2_C5$NAME)), 
                NEN_Huh7 =as.character(unique(NEN_H7_C5$NAME)),
                NEN_PDX =as.character(unique(NEN_PDX_C5$NAME)),
                Niclo_HepG2 =as.character(unique(Niclo_G2_C5$NAME)),
                Niclo_Huh7 =as.character(unique(Niclo_H7_C5$NAME))) # ,C=unique(set1$GeneID[set1$up_down=="down"]), D=unique(set2$GeneID[set2$up_down=="down"]))
setlist2 <- setlist[1:5]
OLlist2 <- overLapper(setlist=setlist2, sep="_", type="vennsets")
OLlist2$Venn_List
counts <- sapply(OLlist2$Venn_List, length); 
pdf("../../microarray/gsea/common_go_neg.pdf")
vennPlot(counts=counts) 
dev.off()

NEN_PDX_C5 = NEN_PDX_C5[order(NEN_PDX_C5$FDR.q.val),]
subset(NEN_PDX_C5, NAME %in% common_pathways)$NAME

names(NEN_G2_C5) = c("NAME", "ES_NEN_HepG2", "NES_NEN_HepG2", "NOM.p.val_NEN_HepG2", "FDR.q.val_NEN_HepG2")
names(Niclo_G2_C5) = c("NAME", "ES_Niclo_HepG2", "NES_Niclo_HepG2", "NOM.p.val_Niclo_HepG2", "FDR.q.val_Niclo_HepG2")
names(NEN_PDX_C5) = c("NAME", "ES_NEN_PDHX", "NES_NEN_PDHX", "NOM.p.val_NEN_PDHX", "FDR.q.val_NEN_PDHX")

merged_common_canonical_pathway_neg = merge(merge(NEN_G2_C5,Niclo_G2_C5, by="NAME" ), NEN_PDX_C5, by="NAME")
merged_common_canonical_pathway_neg = merged_common_canonical_pathway_neg[order(merged_common_canonical_pathway_neg$FDR.q.val_NEN_PDHX),]
write.csv(merged_common_canonical_pathway_neg, "../../microarray/gsea/common_go_neg.csv")


#GO pos
NEN_G2_C5= read.csv("../../microarray/gsea/NEN_G2_c5.GseaPreranked.1441689122447/gsea_report_for_na_pos_1441689122447.csv")
NEN_H7_C5  = read.csv("../../microarray/gsea/NEN_G2_c5.GseaPreranked.1441689122447/gsea_report_for_na_pos_1441689122447.csv")
NEN_PDX_C5 = read.csv("../../microarray/gsea/NEN_PDX_c5.GseaPreranked.1441688600455/gsea_report_for_na_pos_1441688600455.csv")
Niclo_G2_C5 = read.csv("../../microarray/gsea/Niclo_G2_c5.GseaPreranked.1441689438618/gsea_report_for_na_pos_1441689438618.csv")
Niclo_H7_C5 = read.csv("../../microarray/gsea/Niclo_h7_c5.GseaPreranked.1441690000502/gsea_report_for_na_pos_1441690000502.csv")
...
OLlist2$Venn_List
counts <- sapply(OLlist2$Venn_List, length); 
pdf("../../microarray/gsea/common_go_pos.pdf")
vennPlot(counts=counts) 
dev.off()
...
merged_common_canonical_pathway_neg = merge(merge(NEN_G2_C5,Niclo_G2_C5, by="NAME" ), NEN_PDX_C5, by="NAME")
merged_common_canonical_pathway_neg = merged_common_canonical_pathway_neg[order(merged_common_canonical_pathway_neg$FDR.q.val_NEN_PDHX),]
write.csv(merged_common_canonical_pathway_neg, "../../microarray/gsea/common_go_pos.csv")



####
#overlap with others
targets = c("CCND1", "GSK3B", "KDR", "MCL1", "PRKCQ", "STAT3", "AKT1", "CSNK1E", "MYC", "TPI1", "MTOR", "EGFR", "CTNNB1")
data2 = read.csv("../../microarray//Group3/data2.csv")
pdx = subset(data2, GeneSymbol %in% targets, select = c("TargetID", "GeneSymbol","Test.Control.fc", "Test.Control.adj.pval", "fail.count"))

g2_data2 = read.csv("../../microarray//Group1/data2.csv")
g2 = subset(g2_data2, GeneSymbol %in% targets, select = c("TargetID", "G2.NEN.G2.fc", "G2.Niclo.G2.fc", "fail.count" ))

h7_data2 = read.csv("../../microarray//Group2/data2.csv")
h7 = subset(h7_data2, GeneSymbol %in% targets, select = c("TargetID", "H7.NEN.H7.fc", "H7.Niclo.H7.fc", "fail.count" ))

all = merge(merge(pdx, g2, by = "TargetID"), h7, by = "TargetID")
all = all[order(all$GeneSymbol),]
write.csv(all, "../../microarray/target_expr.csv")
