...
library("GSVA")
library("colorRamps")
hcc_drugs <- read.csv("raw/clinicaltrial//hcc_drugs.csv", stringsAsFactors = F)

database <- "cmap"

if (database == "cmap"){
  #CMap results
  load("LIHC/drug/cmap_predictions.RData")
  drug_preds <- results[[1]]
  dz_sig <- results[[2]]
  cmap_experiments <- read.csv("raw/cmap/cmap_drug_experiments.csv")
  valid_instances <- read.csv("raw/cmap/cmap_valid_instances.csv")
  ...
  #LINCS
  load("LIHC/drug/lincs_predictions.RData")
