...
createGGPlot <- function(currPheno, measureVar, groupVars, sig_cutoff =0, main="GGPLot", ylab="pass ylab value", xlab="pass xlab value") {
  ...
  a = ggplot() +
    ...
    geom_hline(yintercept = sig_cutoff, linetype = "longdash")
  return (a)
}
...
query <- paste("select experiment_id, name from proj_repositioning.v_cmap_hits_valid  where  subset_comparison_id = 'LIHC_final_v1' order by cmap_score  "    )         
rs <- dbSendQuery(con, query)    
...
#cluster genes
gene_annot <- read.table("LIHC_dz_signature_cmap_final_v1.txt",header=T,sep="\t") 
...
pheatmap(-drug_dz_signature_rotate, col = greenred(100) , cellheight = 12,  cellwidth = 10, show_rownames = T, legend=T,  filename = "gene_reversed_two_color.pdf") #,  filename = "reversibility.pdf"
...
gene_annot <- read.table("LIHC_dz_signature_lincs_final.txt",header=T,sep="\t") 
gene_annot <- subset(gene_annot, select = c("Symbol", "GeneID"))
...
drug_dz_signature_rotate = t(drug_dz_signature_reversed)
pheatmap(-drug_dz_signature_rotate, col = greenred(100) , cellheight = 12,  cellwidth = 10, show_rownames = T, legend=T,  filename = "lincs_gene_reversed_two_color.pdf") #,  filename = "reversibility.pdf"
...
createGGPlot(scores, measureVar="score", groupVars = "group", sig_cutoff, main = "", ylab = "cmap score", xlab = "")
dev.off()


#find common drugs 
query <- paste("select * from proj_repositioning.v_cmap_hits_valid  where cmap_score < 0 and valid=1 and subset_comparison_id = 'lihc_final_v1' and q_value < 0.01 order by cmap_score  "    )         
rs <- dbSendQuery(con, query)    
...
rs <- dbSendQuery(con, paste("select experiment_id, cmap_score, q_value,q_value, sig_id,pert_desc, pert_time,pert_dose, pert_iname, cell_id from proj_lincs.v_lincs_predictions_drugs where cmap_score<0 and q_value < 0.01 and subset_comparison_id ='", "lihc_final", "' order by cmap_score", sep=""))    
lincs_results <- fetch(rs, n = -1)
write.csv(lincs_results, "lincs_results.csv")

rs <- dbSendQuery(con, paste("select experiment_id, cmap_score, q_value,q_value, sig_id,pert_desc, pert_time,pert_dose, pert_iname, cell_id from proj_lincs.v_lincs_predictions_all where cell_id in ('HEPG2', 'HUH7') and cmap_score<0 and q_value < 0.01 and subset_comparison_id ='", "lihc_final", "' order by cmap_score", sep=""))    
