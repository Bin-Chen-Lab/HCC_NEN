#validate reduced disease signatures using external datasets
...
res <- subset(res, GeneID %in% landmark$gene_id)
...
write.csv(performances,paste(cancer, "/dz_sig/", "performances_reduced.csv", sep=""))

#performances <- read.csv("performances_reduced.csv")

#this shows fc = 2 performs the best
pdf(paste(cancer, "/dz_sig/", "/fc_auc_reduced.pdf", sep=""))
...
pdf(paste(cancer, "/dz_sig/", "p_auc_reduced.pdf", sep=""))
...
pdf(paste(cancer, "/dz_sig/", "p_auc_median_reduced.pdf", sep=""))
  h <- ggplot(a_order, aes(log(a_order$dz_padj_cutoff, 10), roc))
  h + geom_line() + geom_point() + xlab("log10 p adjust") + ylab("AUC")
dev.off()

a <- aggregate(roc ~ dz_fc_cutoff + dz_padj_cutoff, performances, median)
a <- a[order(a$roc),]
a
#best cutoffs: 1.0, 1E-5
